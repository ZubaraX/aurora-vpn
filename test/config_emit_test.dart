...
void main() {
  ...
  final hy2 = parser.parseLink('hy2://pw@h2.example.com:443?sni=h2.example.com#H2')!;
  ...
  void emit(String name, dynamic node, VpnSettings s, {List<dynamic> nodes = const []}) {
    for (final android in [true, false]) {
      final cfg = SingBoxConfigBuilder(isAndroid: android)
          .buildString(node, s, nodes: nodes.cast());
      File('${outDir.path}/${name}_${android ? 'android' : 'windows'}.json')
  ...
  emit('zone_server', reality,
      VpnSettings(domainZoneRules: {'ru': ws.id}), nodes: [ws]);
  emit('perapp_allow', reality,
      const VpnSettings(perAppMode: PerAppMode.allowlist,
